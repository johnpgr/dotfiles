...
test("default config validates and unknown properties fail", () => {
    ...
    assert.equal(validateConfig({ ...DEFAULT_CONFIG, surprise: true }).ok, false);
    assert.equal(
        validateConfig({
            ...DEFAULT_CONFIG,
            delegates: {
...
test("invalid global config falls back atomically", async () => {
    ...
    try {
        await writeFile(path.join(directory, "pi-delegates.json"), '{"maxConcurrent":0}');
...
test("launch parser handles profiles, separators, and dangerous opt-in", () => {
    ...
    const dangerous = parseLaunchArguments("--plan --dangerous task");
    assert.equal(dangerous.ok, false);
    if (!dangerous.ok) assert.match(dangerous.error.message, /requires/);
    const unknown = parseLaunchArguments("--wat task");
    assert.equal(unknown.ok, false);
    if (!unknown.ok) assert.match(unknown.error.message, /Unknown/);
