...
import { indexDelegateArtifacts, searchDelegateArtifacts } from "../src/context-mode.ts";
import type { ArtifactPaths, DelegateJobMetadata } from "../src/types.ts";
...
test("Context Mode uses project/source labels and indexing failure is non-fatal", async () => {
    ...
    try {
        ...
        const search = await searchDelegateArtifacts(fixture, metadata, artifacts, "needle", 1024);
