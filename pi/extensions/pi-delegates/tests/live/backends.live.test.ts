...
import { resolveExecutable } from "../../src/config.ts";
import type { DelegateName, DelegateProfile, DelegateTask } from "../../src/types.ts";
    ...
    async () => {
        ...
        try {
            ...
            const pairs = await Promise.all(
                names.map(async (name) => [name, await resolveExecutable(name)] as const),
            );
            ...
            for (const name of names) {
                for (const profile of ["plan", "implementation"] satisfies DelegateProfile[]) {
                    ...
                    clearTimeout(timer);
                    assert.equal(outcome.ok, true, `${name}/${profile}`);
                    if (outcome.ok) assert.equal(outcome.exitCode, 0, `${name}/${profile}`);
