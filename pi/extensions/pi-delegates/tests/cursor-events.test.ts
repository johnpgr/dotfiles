...
test("Cursor preserves raw events, tolerates unknown data, and writes canonical result", async () => {
    ...
    try {
        ...
        const outcome = await new CursorBackend(fixture).start(task, new AbortController().signal);
        assert.equal(outcome.ok, true);
        if (outcome.ok) assert.equal(outcome.exitCode, 0);
        assert.equal(await readFile(task.artifacts.final, "utf8"), "final cursor answer");
