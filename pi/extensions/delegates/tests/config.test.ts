...
import { parseLaunchArguments, parseSearchArguments } from "../src/args.js";
...
test("default config validates and unknown properties fail", () => {
  ...
  assert.equal(
    validateConfig({ ...DEFAULT_CONFIG, surprise: true }).success,
    false,
  );
  assert.equal(
    validateConfig({
...
test("invalid global config falls back atomically", async () => {
  ...
  try {
    await writeFile(
      path.join(directory, "delegates.json"),
      '{"maxConcurrent":0}',
    );
    assert.deepEqual(
      await loadConfig((warning) => warnings.push(warning)),
      DEFAULT_CONFIG,
    );
...
test("launch parser handles profiles, separators, and dangerous opt-in", () => {
  ...
  assert.deepEqual(
    parseLaunchArguments("--implementation --dangerous -- --starts-with-dash"),
    {
  ...
  assert.throws(
    () => parseLaunchArguments("--plan --dangerous task"),
    /requires/,
  );
