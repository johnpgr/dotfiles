...
import { DelegateManager } from "../src/manager.js";
import type {
  DelegateBackend,
  DelegateName,
  DelegateTask,
...
class FakeBackend implements DelegateBackend {
  ...
  async start(task: DelegateTask, signal: AbortSignal) {
    ...
    return { exitCode: 0, changedFiles: [] };
  }
}
...
test("scheduler enforces global concurrency and per-cwd implementation serialization", async () => {
  ...
  try {
    ...
    await eventually(() =>
      manager.list().every((job) => job.settledAt !== undefined),
    );
...
test("stopping a queued job settles it exactly once", async () => {
  ...
  try {
    ...
    await eventually(
      () => manager.get(first.id)?.metadata.settledAt !== undefined,
    );
