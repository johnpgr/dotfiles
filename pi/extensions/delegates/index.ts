...
import { readBoundedFile } from "./src/artifacts.ts";
...
async function readFinalOrError(filePath: string, maxBytes: number, fallback: string) {
    return readBoundedFile(filePath, maxBytes).catch(() => ({
        text: fallback,
        truncated: false,
    }));
}
...
export default function delegatesExtension(pi: ExtensionAPI) {
    ...
    const appendCompletion = async (metadata: ReturnType<DelegateManager["list"]>[number]) => {
        if (!sessionContext) return;
        const artifacts = artifactForMetadata(metadata);
        const result = await readFinalOrError(
            artifacts.final,
    ...
    const getManager = (ctx: ExtensionContext | ExtensionCommandContext) => {
        managerPromise ??= (async () => {
            ...
            executables = Object.fromEntries(pairs) as Record<DelegateName, string | undefined>;
            ...
            // Clear so the next /delegate-* command can retry after a failed recover().
    ...
    const launch = async (name: DelegateName, rawArgs: string, ctx: ExtensionCommandContext) => {
        try {
            ...
            if (!delegateConfig.enabled) throw new Error(`${name} delegation is disabled.`);
            if (!executables?.[name]) throw new Error(`${name} executable was not found.`);
            ...
            if (name === "agy" && args.profile === "implementation" && !args.dangerousBypass) {
                throw new Error(
    ...
    pi.on("session_start", async (_event, ctx) => {
        sessionContext = ctx;
        await getManager(ctx).catch((error) => ctx.ui.notify(errorText(error), "error"));
    });
