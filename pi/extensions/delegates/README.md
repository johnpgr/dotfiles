...
The extension registers commands only. It registers no tools, so the parent model cannot start, stop, or read a job. Every job starts from a slash command that you type.
...
1. Run `npm install` in this directory. The `prepare` script compiles the TypeScript sources into `dist/`.
...
2. The delegate needs `allowDangerousBypass: true` in `delegates.json`. Only `agy` has this by default.
...
The extension reads `$PI_AGENT_DIR/delegates.json`. The file is optional.
...
| `npm run check` | Type-checks the sources and the tests. |
...
| Path | Role |
|---|---|
| `index.ts` | Registers the commands and holds the session state. |
| `src/manager.ts` | Schedules jobs, tracks state, and runs the timeouts. |
...
| `src/config.ts` | Loads `delegates.json` and finds executables. |
