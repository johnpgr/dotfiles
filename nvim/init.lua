-- Single-file Neovim config using vim.pack
...
package.preload['statusline'] = function()
    local ignored_ls = {
        "opencode",
        "copilot",
    }
...
package.preload['keymaps'] = function()
...
package.preload['cmdline'] = function()
...
package.preload['utils'] = function()
    ...
    M.is_kitty = os.getenv("TERM") == "xterm-kitty" or os.getenv("TERM") == "xterm-ghostty"
...
package.preload['lookup'] = function()
    ...
    -- Get provider using vim.ui.select/refer picker
    local function get_provider(query, on_choice)
        vim.ui.select(M.providers, {
...
package.preload['colorscheme'] = function()
    local M = {}
...
package.preload['plugins.init'] = function()
    -- General plugins
    return {
        {
...
package.preload['plugins.blink'] = function()
    -- Blink completion
    return {
        "saghen/blink.cmp",
        event = "InsertEnter",
...
package.preload['plugins.dap'] = function()
...
package.preload['plugins.git'] = function()
...
package.preload['plugins.refer'] = function()
    local uv = vim.uv or vim.loop
    ...
    local function open_lazy_data_files()
        pick_files_fff_in_dir(vim.fs.joinpath(vim.fn.stdpath("data"), "site", "pack"), "Pack Data Files > ")
    end
...
package.preload['plugins.neotree'] = function()
    return {
        {
...
package.preload['plugins.fff'] = function()
    return {
...
package.preload['plugins.db'] = function()
...
package.preload['plugins.cord'] = function()
    return {
        "vyfor/cord.nvim",
        ---@type CordConfig
        opts = {
...
package.preload['plugins.conform'] = function()
    -- Formatter plugins
    return {
...
package.preload['plugins.compile_mode'] = function()
    return {
...
package.preload['plugins.which_key'] = function()
    -- WhichKey.nvim
    return {
...
package.preload['plugins.image'] = function()
...
package.preload['plugins.align'] = function()
    return {
...
package.preload['plugins.quickfix'] = function()
    -- Quickfix
    return {
...
package.preload['plugins.snippets'] = function()
    -- Snippets
    return {
        {
            "L3MON4D3/LuaSnip",
...
package.preload['user.java_ftplugin'] = function()
...
package.preload['lsp.clangd'] = function()
    return {
        cmd = { "/opt/llvm-22.1.0/bin/clangd" },
    }
end
...
local plugin_module_names = {
    "plugins.init",
    "plugins.align",
    "plugins.blink",
    "plugins.compile_mode",
    "plugins.conform",
    "plugins.copilot",
    "plugins.cord",
    "plugins.dap",
...
for _, module_name in ipairs(plugin_module_names) do
    local mod = require(module_name)
    if type(mod) == "table" then
        if type(mod[1]) == "string" then
            table.insert(top_level_specs, mod)
        else
            for _, spec in ipairs(mod) do
                table.insert(top_level_specs, spec)
            end
        end
    end
end
...
local function spec_enabled(spec)
    ...
    if type(spec.cond) == "boolean" and not spec.cond then
        return false
    end
    return true
end
...
local function spec_key(spec)
    if type(spec) == "string" then
        return (spec:gsub("%.git$", ""):match("[^/]+$"))
    end
    local src = spec.name or spec[1] or spec.src or ""
...
local function register_spec(spec)
    ...
    local current = spec_registry[key]
    if current then
        ...
        if spec.dependencies then
            current.dependencies = current.dependencies or {}
            for _, dep in ipairs(spec.dependencies) do
                table.insert(current.dependencies, dep)
            end
        end
    else
        spec_registry[key] = shallow_copy(spec)
    end
...
for _, spec in ipairs(top_level_specs) do
    register_spec(spec)
end
...
local pack_specs = {}
local pack_specs_seen = {}
local function append_pack_spec(spec)
    if type(spec) == "string" then
        spec = spec_registry[spec_key(spec)]
    else
        spec = spec_registry[spec_key(spec)] or spec
    end
    if not spec_enabled(spec) then
        return
    end
    if spec.dependencies then
        for _, dep in ipairs(spec.dependencies) do
            append_pack_spec(dep)
        end
    end

    local key = spec_key(spec)
    if pack_specs_seen[key] then
        return
    end

    local src = spec[1] or spec.src
    if src then
        local pack_spec = { src = gh(src) }
        if spec.name then
        ...
        pack_specs_seen[key] = true
    end
end

for _, spec in ipairs(top_level_specs) do
    append_pack_spec(spec)
end

vim.pack.add(pack_specs, { load = true, confirm = false })

local function setup_opts_only(spec)
    local repo = spec[1] or spec.src
    if repo == "vyfor/cord.nvim" then
        require("cord").setup(spec.opts)
    elseif repo == "3rd/image.nvim" then
        require("image").setup(spec.opts)
    elseif repo == "saghen/blink.compat" then
        require("blink.compat").setup(spec.opts)
    end
end
...
local function apply_key(entry)
    ...
    opts.mode = nil
    vim.keymap.set(mode, entry[1], entry[2], opts)
end

local keyed = {}
local function apply_spec_keys(spec)
    if type(spec) == "string" then
        spec = spec_registry[spec_key(spec)]
    else
        spec = spec_registry[spec_key(spec)] or spec
...
for _, spec in ipairs(top_level_specs) do
    apply_spec_keys(spec)
end

vim.api.nvim_create_autocmd("FileType", {
    pattern = "java",
    callback = function()
        require("user.java_ftplugin")
    end,
})
